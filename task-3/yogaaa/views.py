...
def yoga(request):
    ...
    categories = Category.objects.all() #получаем категории для выпадающих списков из базы данных
    manufacturers = Manufacturer.objects.all() #получаем производителей для выпадающих списков из базы данных
...
def product_list(request):
    products = Product.objects.all() #получаем базовый набор всех товаров из базы данных
    query = request.GET.get('q') #получаем строку поиска от пользователя из запроса
    category_id = request.GET.get('category') #получаем фильтр по категории из запроса
    manufacturer_id = request.GET.get('manufacturer') #получаем фильтр по производителю из запроса
    ...
    if category_id:
        products = products.filter(category_id=category_id) #выполняем фильтрацию по категориям по заданию

    if manufacturer_id:
        products = products.filter(manufacturer_id=manufacturer_id) #выполняем фильтрацию по производителям по заданию

    categories = Category.objects.all() #получаем все категории товаров из базы данных
    manufacturers = Manufacturer.objects.all() #получаем всех производителей товаров из базы данных
    return render(request, 'shop/product_list.html', {
        'products': products,
        'categories': categories,
        'manufacturers': manufacturers
    }) #открываем страницу списка товаров каталога в папке shop по заданию


def product_detail(request, pk):
    product = get_object_or_404(Product, id=pk) #отображение информации о товаре с обработкой ошибки 404 по заданию
    return render(request, 'shop/product_detail.html', {'product': product}) #открываем страницу детальной информации о товаре в папке shop по заданию


def add_to_cart(request, product_id):
    cart = get_or_create_test_cart() #получаем личную корзину текущего покупателя
    product = get_object_or_404(Product, id=product_id) #находим покупаемый товар или возвращаем ошибку 404 по заданию
    cart_item, created = CartItem.objects.get_or_create(cart=cart, product=product, defaults={'quantity': 0}) #создаем или находим элемент
...
def update_cart(request, item_id):
    error_message = None #инициализируем пустую переменную для сообщения об ошибке
    if request.method == 'POST':
        ...
        try:
            ...
            if 'quantity' in e.message_dict:
                error_message = e.message_dict['quantity'] #перехватываем текст ошибки валидации остатков со склада
            else:
                error_message = "Ошибка валидации количества!" #записываем общую ошибку валидации

    cart = get_or_create_test_cart() #получаем текущую корзину со всеми элементами
    return render(request, 'shop/cart.html', {'cart': cart, 'error_message': error_message}) #открываем страницу корзины в папке shop с ошибкой по заданию


def remove_from_cart(request, item_id):
    cart_item = get_object_or_404(CartItem, id=item_id) #находим элемент корзины по его уникальному номеру id по заданию
...
def cart_view(request):
    ...
    return render(request, 'shop/cart.html', {'cart': cart}) #отображение всех элементов корзины с общей стоимостью в папке shop по заданию


def about(request):
    return render(request, 'about.html') #открываем страницу информации об авторе сайта


def info(request):
    return render(request, 'info.html') #открываем страницу общей информации о магазине йоги