...
def checkout_view(request):
    cart = get_or_create_test_cart(request.user) #получаем корзину текущего пользователя для оформления заказа по заданию
    cart_items = cart.items.all() #считываем содержимое корзины по заданию второго пункта лабораторной работы 19

    if request.method == 'POST':
        order = Order.objects.create(user=request.user, total_cost=cart.total_cost) #создаем заказ на основе корзины по заданию лабы 19
        
        wb = openpyxl.Workbook() #инициализируем генерацию чека в формате Excel по заданию второго пункта лабы 19
        ws = wb.active #активируем рабочий лист эксель-файла для записи товарного чека по заданию
        ws.title = "Чек заказа" #задаем имя вкладки электронной таблицы чека заказа по заданию
        
        ws.append(["Чек по заказу №", order.id]) #записываем номер товарного чека в файл Excel по заданию
        ws.append(["Покупатель:", request.user.username]) #записываем имя авторизованного клиента в файл Excel по заданию
        ws.append([]) #вставляем пустую строку для красивой структуры чека в электронной таблице Excel
        ws.append(["Товар", "Цена (BYN)", "Количество", "Стоимость"]) #заголовки колонок товарного чека в Excel по заданию
        
        for item in cart_items:
            OrderItem.objects.create(
                order=order, product=item.product, price=item.product.price, quantity=item.quantity
            ) #переносим товары из корзины в заказ
            ws.append([item.product.name, item.product.price, item.quantity, item.element_cost]) #пишем строки товаров в Excel по заданию
            
        ws.append([]) #вставляем пустую строку перед выводом итоговой суммы
        ws.append(["ИТОГО К ОПЛАТЕ:", "", "", order.total_cost]) #записываем финальную сумму заказа в Excel
        
        excel_file = io.BytesIO() #создаем байтовый поток в оперативной памяти для временного файла Excel по заданию
        wb.save(excel_file) #сохраняем сгенерированный чек Excel в виртуальный файл в оперативной памяти по заданию
        excel_file.seek(0) #сбрасываем указатель потока в начало файла Excel для чтения и отправки данных
        
        user_email = request.user.email if request.user.email else "customer@proyoga.by" #определяем почту покупателя йога-товаров для отправки
        email = EmailMessage(
            f"Ваш чек по заказу №{order.id} — ProYoga",
            "Благодарим за покупку в магазине ProYoga! Ваш чек во вложении.",
            "noreply@proyoga.by", [user_email]
        ) #формируем отправку чека по электронной почте покупателю
        ...
        try:
            email.send(fail_silently=True) #отправляем письмо с вложенным чеком Excel покупателю
        except Exception:
            pass #гасим системные ошибки если локальный почтовый сервер отправки писем не настроен
            
        cart.items.all().delete() #очистка корзины после успешного оформления заказа
        return render(request, 'shop/checkout.html', {'order': order, 'success': True}) #выводим страницу успешного оформления заказа ковриков

    return render(request, 'shop/checkout.html', {'cart': cart}) #открываем страницу подтверждения заказа с деталями корзины пользователя