...
class Cart(models.Model):
    ...
    def __str__(self):
        return f"Корзина пользователя {self.user.username}" #строка по тз
...
class CartItem(models.Model):
    ...
    def __str__(self):
        return f"{self.product.name} ({self.quantity} шт.)" #строка по тз
    ...
    def clean(self):
        if self.product and self.quantity > self.product.stock:
            raise ValidationError({'quantity': f'Количество не должно превышать остаток на складе ({self.product.stock} шт.)!'}) #валидация количества по тз
