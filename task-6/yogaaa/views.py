...
def profile_page_view(request):
    # Визуальная страница личного кабинета на Bootstrap (Задание 4)
    # Мы не вешаем @login_required, так как обработку анонимов нужно сделать на фронтенде по ТЗ
    return render(request, 'shop/profile.html')

# Обеспечение сценариев 4 и 5: Кастомная защита API-ресурсов каталога товаров
class ProductViewSet(viewsets.ModelViewSet):
    queryset = Product.objects.all()
    serializer_class = ProductSerializer
    ...
    class DjangoIsAdminOrReadOnly(drf_permissions.BasePermission):
        def has_permission(self, request, view):
            # Сценарий 1: Чтение (SAFE_METHODS) разрешено абсолютно всем гостям
            if request.method in drf_permissions.SAFE_METHODS:
                return True
            # Сценарий 4 и 5: Модификация разрешена исключительно роли ADMIN
            return (
                request.user and 
                request.user.is_authenticated and 
                getattr(request.user.profile, 'role', '') == 'ADMIN'
...
# Обеспечение сценария 6: Ролевое разграничение видимости истории заказов
class OrderViewSet(viewsets.ModelViewSet):
    serializer_class = OrderSerializer
    permission_classes = [drf_permissions.IsAuthenticated]
    ...
    def get_queryset(self):
        user = self.request.user
        # Сценарий 5: Администратор видит абсолютно все оформленные заказы студии
        if getattr(user.profile, 'role', '') == 'ADMIN':
            return Order.objects.all()
        # Сценарий 6: Обычный покупатель изолирован строго внутри своих личных заказов
        return Order.objects.filter(user=user)