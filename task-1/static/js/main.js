document.addEventListener('DOMContentLoaded', function() {
    const container = document.getElementById('products-container'); //находим контейнер для динамического вывода товаров йоги
    ...
    if (container) {
        fetch('/api/products/')
            ...
            .then(data => {
                spinner.style.display = 'none'; //скрываем индикатор загрузки после успешного получения json
                if (data.length === 0) {
                    container.innerHTML = '<div class="col-12 text-center py-4 text-muted">Товары отсутствуют</div>';
                    return;
                }
                ...
                `).join(''); //динамически рендерим карточки товаров йоги из полученного массива api
            })
            .catch(error => {
                spinner.style.display = 'none'; //скрываем спиннер в случае возникновения ошибки соединения
    ...
    .then(response => {
        alertContainer.innerHTML = `
            <div class="alert alert-success alert-dismissible fade show shadow" role="alert">
                <strong>🛒 Успех!</strong> Товар успешно добавлен в корзину без перезагрузки страницы.
