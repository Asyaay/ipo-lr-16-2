...
def get_or_create_test_cart(user):
    cart, _ = Cart.objects.get_or_create(user=user) #находим или создаем личную корзину конкретного авторизованного пользователя
    return cart
...
def yoga(request):
    ...
    manufacturers = Manufacturer.objects.all() #получаем производители для выпадающих списков из базы данных
...
def product_list(request):
    ...
    return render(request, 'shop/product_list.html', {
        ...
        'categories': categories,
        'manufacturers': manufacturers
    }) #открываем страницу списка товаров каталога


def product_detail(request, pk):
    product = get_object_or_404(Product, id=pk) #отображение информации о товаре с обработкой ошибки 404 по заданию
    return render(request, 'shop/product_detail.html', {'product': product}) #открываем страницу детальной информации о товаре


@login_required #ограничиваем доступ к добавлению в корзину только для авторизованных пользователей по заданию
def add_to_cart(request, product_id):
    cart = get_or_create_test_cart(request.user) #получаем личную корзину текущего вошедшего пользователя по заданию
...
def update_cart(request, item_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            cart_item.full_clean() #запускаем валидацию количества против остатка на складе
    ...
    cart = get_or_create_test_cart(request.user) #получаем текущую корзину авторизованного пользователя
    return render(request, 'shop/cart.html', {'cart': cart, 'error_message': error_message}) #открываем страницу корзины с ошибкой
...
def cart_view(request):
    cart = get_or_create_test_cart(request.user) #получаем корзину текущего авторизованного пользователя по заданию
    return render(request, 'shop/cart.html', {'cart': cart}) #отображение всех элементов корзины с общей стоимостью по заданию
